...
# Answer instruction is keyed by dataset_type. Both math and mcqa stack the
# \boxed{} instruction on top of whatever the JSONL question already contains
# (for mcqa the JSONL row already ends with "Your final answer should only
# contain the letter..." so the model sees both — they reinforce, per DeepConf
# paper Appendix F p. 24). Keep the dict so future dataset types can diverge.
...
def main():
    ...
    if args.model_type == "gpt":
        prompt = prepare_prompt_gpt(question, deep_llm.tokenizer, dataset_type=args.dataset_type)
    else:
        prompt = prepare_prompt(question, deep_llm.tokenizer, args.model_type, dataset_type=args.dataset_type)
